...
public class SVGMigrationContribution extends AbstractMigrationContribution {
  ...
  static {
    ...
    // differentSizeTemp.put("/org.polarsys.capella.core.sirius.analysis/description/images/FunctionDuplicateDiagram.svg",
    // new Dimension(56,55));
    // differentSizeTemp.put("/org.polarsys.capella.core.sirius.analysis/description/images/FunctionGatherDiagram.svg",
    // new Dimension(56,56));
    // differentSizeTemp.put("/org.polarsys.capella.core.sirius.analysis/description/images/FunctionRouteDiagram.svg",
    // new Dimension(56,55));
    // differentSizeTemp.put("/org.polarsys.capella.core.sirius.analysis/description/images/FunctionSelectDiagram.svg",
    // new Dimension(56,56));
    // differentSizeTemp.put("/org.polarsys.capella.core.sirius.analysis/description/images/FunctionSplitDiagram.svg",
    // new Dimension(56,56));
    ...
    Map<String, Map<String, Dimension>> differentSizePerDiagram = new HashMap<>();
    Map<String, Dimension> differentSizeTemp = new HashMap<>();
    differentSizeTemp.put("Data Flow Blank", new Dimension(55, 56));
  ...
  public void unaryMigrationExecute(EObject currentElement, MigrationContext context) {
    ...
    if (isValidModelVersion(context) && currentElement instanceof AbstractDNode) {
      AbstractDNode node = ((AbstractDNode) currentElement);
      Style elementStyle = node.getStyle();
      ...
      if (elementStyle instanceof WorkspaceImage) {
        ...
        changeSizeOf(node, image.getWorkspacePath());
      }
    }
  }

  private void changeSizeOf(AbstractDNode currentElement, String newWorkspacePath) {
    if (DIFFERENT_IMAGE_SIZE_PER_DIAGRAM.containsKey(newWorkspacePath)) {
      DDiagram diagram = currentElement.getParentDiagram();
      Dimension dimension = getNewDimensionOfElementWithinDiagram(diagram, newWorkspacePath);
      Node node = getNode(diagram, currentElement);
      if (node != null && node.getLayoutConstraint() instanceof Bounds) {
        Bounds bounds = (Bounds) (node.getLayoutConstraint());
        // If user hasn't changed the size by himself, we force to the previous size
        if (bounds != null && bounds.getWidth() == -1 && bounds.getHeight() == -1) {
          bounds.setWidth(dimension.width());
          bounds.setHeight(dimension.height());
        }
        // If we don't set '4' in here like defined in Odesign, Sirius will set back the size at the first refresh.
  ...
  private Node getNode(DDiagram diagram, AbstractDNode currentElement) {
